...
bool a_play ( llint n , llint m , mpb& a_memo , mpb& b_memo ) {
    if ( n == m or n == 1 or m == 1 )   return true ;
    ...
    a_memo.insert ( pair<pair<llint,llint>,bool> ( pair<llint,llint>(n,m) , result ) );
    return result ; 
}
...
bool r_play ( llint n , llint m , mpb& a_memo , mpb& b_memo ) {
    ...
    b_memo.insert ( pair<pair<llint,llint>,bool> ( pair<llint,llint>(n,m) , result ) );
    return result ; 
}

string calc ( llint n , llint m ) {
    mpb a_memo , b_memo ; 
    if ( a_play ( n , m , a_memo , b_memo ) )   return string("Ari") ;
    else return string("Rich");
    return string();
}
