...
ld calc(llint& n, vector<point>& input) {
  ...
  for (llint i = 0; i < n; i++) {
    ...
    for (llint j = i + 1; j < n; j++) {
      ...
      llint y_values = compare.second - current.second,
            x_values = compare.first - current.first, slope = 0;
      if (abs(x_values) == abs(y_values))
        return 0;
      else
        min_distance =
            min(min_distance, min(abs(compare.first - compare.second -
                                      current.first + current.second),
                                  abs(compare.first + compare.second -
                                      current.first - current.second)));
    }
  }
