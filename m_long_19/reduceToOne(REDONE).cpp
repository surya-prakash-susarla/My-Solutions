...
pair<llint,llint> findClosest ( llint required ) {
    if ( prev_values.size() == 0 or (prev_values.begin())->first > required )  return pair<llint,llint>(-1,-1) ;
    if ( (prev_values.begin())->first == required )  return pair<llint,llint> ( required , prev_values.begin()->second );
