...
class big_int {
    public:
        ...
        big_int operator+ ( const big_int &right_value ){
            ...
            if ( increment > 0 )    result.push_back(char(increment+'0'));
        ...
        big_int single_char_mul ( string input , char mul_dig ){
            if ( mul_dig == '0' )   return string(input.size(),'0');
            ...
            if(increment >0)    final_answer.push_back(char(increment+'0'));
        ...
        big_int min ( big_int value ){
            if ( this->value.size() < value.value.size() )  return *this;
            else if ( this->value.size() > value.value.size() ) return value;
            else {
                int i=0 ;
                while ( i<this->value.size() ){
                    if ( this->value[i]-'0' < value.value[i]-'0' )  return *this;
                    else if ( this->value[i]-'0' > value.value[i]-'0' ) return value;
                    i++;
                }
