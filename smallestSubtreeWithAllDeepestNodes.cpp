...
class Solution {
 public:
  pair<node*, int> calc(node* root, int depth) {
    ...
    if (max_depth == -1)
      return {root, depth};
    if (left_value.second == max_depth and right_value.second == max_depth)
      return {root, max_depth};
    if (left_value.second == max_depth)
      return {left_value.first, max_depth};
    if (right_value.second == max_depth)
      return {right_value.first, max_depth};
    return {NULL, -1};
  }
