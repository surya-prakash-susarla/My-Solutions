...
vector<int> djikstraSourceAllPairShortestPath ( int source , vector<vector<int>> graph ) {
    ...
    while ( true ) {
        ...
        for ( int i=0 ; i<graph.size() ; i++ )
            if ( graph[index][i] != -1 and ( distances[i] == -1 or distances[i] > graph[index][i] + distances[index] ) )
