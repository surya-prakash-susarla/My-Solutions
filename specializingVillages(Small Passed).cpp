...
void getCombination(llint index, vector<bool> fruit, vector<llint> &values, vector<vector<llint>> &edges)
{
    if (index == edges.size() - 1)
    {
        ...
        for (llint i = 0; i < edges.size(); i++)
        {
            ...
            for (llint j = 0; j < edges.size(); j++)
            {
                if (edges[i][j] != max_value and fruit[j] != fruit[i] and edges[i][j] < nearest)
                    nearest = edges[i][j];
            }
    ...
    for (llint i = 0; i < nodes; i++)
    {
        for (llint j = i; j < nodes; j++)
        {
            ...
            for (llint k = 0; k < nodes; k++)
                if (edges[i][k] != max_value and edges[k][j] != max_value and edges[i][j] > edges[i][k] + edges[k][j])
                    edges[i][j] = edges[i][k] + edges[k][j], edges[j][i] = edges[i][k] + edges[k][j];
        }
    }
