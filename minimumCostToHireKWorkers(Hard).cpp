...
// idea : calculate the ratio of all the workers using wage/quality. Store the
// wages in ascending order. For each worker in the map
//        take the lowest k-1 workers with ratio less than the current worker.
//        Store the lowest K workers in a multiset since the quality values may
//        be repeated. Lowest such value is the answer.
 ...
 public:
  double mincostToHireWorkers(vector<int>& quality, vector<int>& wage, int k) {
    ...
    while (it != ratioValues.end()) {
      ...
      // then there is no advantage in replacing them since the ratio is already
      // higher
      currentValue = *it;
