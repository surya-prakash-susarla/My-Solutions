...
public:
    ...
    vector<int> pancakeSort(vector<int>& input) {
        ...
        for ( int i=input.size()-1 ; i > 0 ; i-- ) {
            ...
            auto end = input.begin() + max_value.second + 1 > input.end() ? input.end() : input.begin() + max_value.second + 1 ;
            reverse ( input.begin() , end ) ;
            auto part = input.begin() + i + 1 > input.end() ? input.end() : input.begin() + i + 1 ;
