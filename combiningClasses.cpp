...
//  Range :  pair<llint,llint> : pair.first = higher limit ; pair.second = lower
//  limit
...
llint calc(llint location,
           vector<pair<llint, llint>> ranges,
           llint total_students) {
  if (location > total_students)
    return 0;
  llint max_upper_value = ranges[0].first,
        max_lower_value = ranges[ranges.size() - 1].second;
  llint answer = -1, check_value = (max_upper_value + max_lower_value) / 2,
        prev_location = -1, prev_answer = -1;
...
int main() {
  ...
  for (llint k = 1; k <= t; k++) {
    llint n, q;
    cin >> n >> q;
    llint x1, x2, y1, y2, z1, z2, a1, a2, a3, b1, b2, b3, c1, c2, c3, m1, m2,
        m3;
    cin >> x1 >> x2 >> a1 >> b1 >> c1 >> m1 >> y1 >> y2 >> a2 >> b2 >> c2 >>
        m2 >> z1 >> z2 >> a3 >> b3 >> c3 >> m3;
    ...
    for (llint i = 2; i < n; i++) {
      x_values.push_back((a1 * x_values[i - 1] + b1 * x_values[i - 2] + c1) %
                         m1);
      y_values.push_back((a2 * y_values[i - 1] + b2 * y_values[i - 2] + c2) %
                         m2);
    }
    vector<pair<llint, llint>> ranges;
    llint total_students = 0;
    for (llint i = 0; i < n; i++) {
      ranges.push_back(make_pair((max(x_values[i], y_values[i]) + 1),
                                 (min(x_values[i], y_values[i]) + 1)));
    ...
    for (llint i = 2; i < q; i++) {
      z_values.push_back((a3 * z_values[i - 1] + b3 * z_values[i - 2] + c3) %
                         m3);
