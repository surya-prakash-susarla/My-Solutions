...
llint dfs(llint node, vector<bool> &visited, vector<vector<bool>> matrix, vector<int> cost)
{
...
llint balancedForest(vector<int> c, vector<vector<int>> edges)
{
    ...
    for (llint i = 0; i < edges.size(); i++)
        matrix[edges[i][0] - 1][edges[i][1] - 1] = true, matrix[edges[i][1] - 1][edges[i][0] - 1] = true;
    llint total_sum = 0;
