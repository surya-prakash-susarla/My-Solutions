...
// The comparator is specifically written to comply with c++17. Might cause errors in other verisons.
// C++11 version comparator
// struct stk_comp { 
...
struct stk_comp { 
    bool operator() ( const std::stack<int>* a , const std::stack<int>* b ) const {
        if ( a->size() == b->size() )   answer = a->top() > b->top() ; 
