...
llint find ( int index , llint current_color , llint seen , llint time , set<llint>& colors , 
                        map<llint,map<llint,llint>>& loc , llint& k ) {
    ...
    // if ( ik != loc.end() )
    //     cout << " VALUE : " << ik ->first << " ";
    // cout << "INDEX : " << index << " , COLOR : " << current_color << " SEEN : " << seen << " TIME : " << time << endl ;
    ...
    if ( nit == loc.end() )
        if ( seen >= k )    return time ;
        else    return numeric_limits<llint>::max() ;

    if ( current_color != -1  )
        con = find ( index+1 , current_color , seen , time + ( nit->first - it->first ) , colors , loc , k );

    if ( colors.size() > 0 )
        for ( set<llint>::iterator sit = colors.begin() ; sit != colors.end() ; sit++ ) {
