...
void __find__(shared_ptr<node> current_node,
              const long_int& group_size,
              long_int& answer_score) {
  if (current_node->count < group_size) {
    return;
  }
  if (current_node->count == group_size &&
      current_node->map_values.size() != 1) {
  ...
  for (unordered_map<char, shared_ptr<node>>::iterator it =
           current_node->map_values.begin();
       it != current_node->map_values.end(); it++) {
...
long_int __calc__() {
  ...
  for_each(values.begin(), values.end(),
           [](string& current_value) { cin >> current_value; });
