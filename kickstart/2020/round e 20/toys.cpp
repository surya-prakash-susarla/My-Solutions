...
constexpr toy DELETED_TOY = {std::numeric_limits<llint>::min(),
                             std::numeric_limits<llint>::min()};

toy __rec__(std::vector<toy> values, llint c_value, llint deleted) {
  llint first_fail = -1;

  for (llint i = 0; i < values.size(); i++)
    if (values[i] != DELETED_TOY &&
        values[i].second > c_value - values[i].first) {
      first_fail = i;
      break;
    }
  ...
  std::for_each(values.begin(), values.begin() + first_fail,
                [&](const toy& current) {
  ...
  return current_max_time > res_toy.second ? toy{deleted, current_max_time}
                                           : res_toy;
}
...
toy __calc__() {
  ...
  std::for_each(values.begin(), values.end(),
                [&](const toy& value) { cumulative_value += value.first; });
