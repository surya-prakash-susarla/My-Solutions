...
class Solution {
 public:
  int maxSatisfaction(vector<llint>& values) {
    vec<std::pair<llint, llint>> memo(values.size(),
                                      {0 /*total value including multiplier*/,
                                       0 /*sum of all values from here*/});
    ...
    if (pos_ind == -1)
      // if all are negative numbers then no point in going forward, just return
      // 0.
      return 0;

    // since the value will certainly include all positive values in the fold,
    // we only go backwards into the negative end to see if that helps.
    // calculate first positive value and then use that rec for all backward
    // calculations.
