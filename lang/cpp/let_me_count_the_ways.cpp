...
llint calc(llint singles, llint married, llint rem_married, bool prev_type) {
  if (singles < 0 or married < 0 or rem_married < 0)
    return 0;
  if (singles == 0 and married == 0 and rem_married == 0)
    return 1;
  ...
  if (married >= 2)
    new_married_placed =
        calc(singles, married - 2, rem_married + 1, true) * married;
  llint prev_married_placed = 0;
  if (prev_type and rem_married > 1)
    prev_married_placed =
        calc(singles, married, rem_married - 1, false) * (rem_married - 1);
  else if (not prev_type and rem_married >= 1)
    prev_married_placed =
        calc(singles, married, rem_married - 1, false) * rem_married;
