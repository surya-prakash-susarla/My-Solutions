...
public:
    ...
    int calc ( int index , int sum_value , int l , int m , const vector<int>& input , const vector<pair<int,int>>& values ) {
        ...
        if ( l != 0 and index+l <= input.size() ) 
            l_value = calc ( index+l , sum_value + values[index].first , 0 , m , input , values  );
                
        // try to fill m value 
        int m_value = -1 ;
        if ( m != 0 and index+m <= input.size() ) 
            m_value = calc ( index+m , sum_value + values[index].second , l , 0 , input , values );
    ...
    int maxSumTwoNoOverlap(const vector<int>& input, int l, int m) {
        ...
        int temp = 0;
        for ( int i=0 ; i<l ; i++ ) temp += input[i];
        values[0].first = temp ;
        ...
        temp = 0 ;
        for ( int i=0 ; i<m ; i++ ) temp += input[i];
