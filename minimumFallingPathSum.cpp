...
public:
    ...
    int calc ( int x , int y , int sum , vector<vector<int>>& memo , const vector<vector<int>>& input ) {
        ...
        memo[x][y] = input[x][y] + min ( calc ( x+1, y, 0 , memo ,input ) , 
                                  min ( calc ( x+1, y-1 , 0 , memo , input ) , calc ( x+1 , y+1 , 0 , memo , input ) ) );
        return memo[x][y];
    }
    ...
    int minFallingPathSum( vector<vector<int>>& input ) {
        ...
        vector<vector<int>> memo ( input.size() , vector<int>(input[0].size() , numeric_limits<int>::max() ) );
