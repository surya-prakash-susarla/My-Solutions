...
public:
    ...
    int movePath ( int count , int x , int y , vector<vector<int>>& grid ) {
        ...
        if ( grid[x][y] == 0 or grid[x][y] == -1 )  return count ;
        if ( x == 0 or y == 0 or x == grid.size()-1 or y == grid[0].size()-1 )
            return -1 ;
        count += 1 ;
        grid[x][y] = -1 ;
        int left = movePath ( 0 , x , y-1 , grid ),
            right = movePath ( 0 , x , y+1 , grid ),
            top = movePath ( 0 , x-1 , y , grid ),
            bottom = movePath ( 0 , x+1 , y , grid );
        if ( left == -1 or right == -1 or top == -1 or bottom == -1 )   return -1 ;
